...
function createInstanceMock() {
  ...
  return {
    event,
    render: vi.fn(async (_container?: HTMLElement, _context?: string) => { event.emit('rendered'); }),
    updateProps: vi.fn(async () => undefined),
    close: vi.fn(async () => { event.emit('close'); }),
  };
}
...
describe('React driver with React DOM', () => {
  ...
  it('recovers from a rejected render when context changes, retaining the forwarded container', async () => {
    ...
    const Wrapped = createReactComponent(factory as unknown as ForgeFrameComponent, { React });
    const ref = React.createRef<HTMLDivElement>();
    const onError = vi.fn();

    await React.act(async () => {
      root.render(React.createElement(Wrapped, { context: 'popup', ref, onError }));
    });
    ...
    await React.act(async () => {
      root.render(React.createElement(Wrapped, { context: 'iframe', ref, onError }));
    });
  ...
  it('reports construction errors locally without unmounting sibling content', async () => {
    ...
    const Component = create({
      ...
      props: { name: { schema: prop.string(), decorate: () => { throw failure; } } },
    });
    const Wrapped = createReactComponent(Component, { React });
    const onError = vi.fn();

    await React.act(async () => {
      root.render(React.createElement('section', null,
    ...
    expect(container.querySelector('span')?.textContent).toBe('Other application content');
  ...
  it('lets lifecycle observers reject without creating unhandled promises', async () => {
    ...
    await React.act(async () => {
      root.render(React.createElement(Wrapped, {
        onRendered: async () => { throw renderedError; },
        onClose: async () => { throw closeError; },
      }));
    });
    await React.act(async () => root.render(null));

    expect(log).toHaveBeenCalledWith('Error in async event handler for "rendered":', renderedError);
    expect(log).toHaveBeenCalledWith('Error in async event handler for "close":', closeError);
  });
