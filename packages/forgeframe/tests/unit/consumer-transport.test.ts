...
const createdTransports: Array<ConsumerTransport<TestProps, { ready: boolean }>> = [];
...
function createTransport({
  ...
  resolveUrlOrigin = (url: string) => new URL(url, window.location.origin).origin,
}: {
...
function createHandlers(): ConsumerTransportHandlers<{ ready: boolean }> & {
  ...
  return {
    ...
    onGetSiblings: vi.fn(
      async (_request): Promise<SiblingInfo[]> => []
    ),
  };
}
...
describe('ConsumerTransport', () => {
  ...
  it('should not clear a newer init promise when an older waitForHost call times out', async () => {
    ...
    const transport = createTransport();
    const onError = vi.fn();
    const waiting = transport.waitForHost(25, 'consumer-transport-component', onError);
