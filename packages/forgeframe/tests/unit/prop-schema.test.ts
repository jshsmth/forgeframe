...
describe("StandardSchemaV1 compliance", () => {
	...
	it("prop.union() should be a valid StandardSchemaV1", () => {
		expect(isStandardSchema(prop.union(prop.string(), prop.number()))).toBe(
			true,
		);
	});
...
describe("prop.date()", () => {
	...
	it("should reject non-Date values", () => {
		...
		expect(schema["~standard"].validate(1711929600000)).toHaveProperty(
			"issues",
		);
	});
	...
	it("should reject invalid Date instances", () => {
		const schema = prop.date();
		expect(schema["~standard"].validate(new Date("invalid"))).toHaveProperty(
			"issues",
		);
	});
	...
	it("should validate inclusive min and max bounds", () => {
		...
		expect(
			schema["~standard"].validate(new Date("2025-12-31T23:59:59.999Z")),
		).toHaveProperty("issues");
		expect(
			schema["~standard"].validate(new Date("2027-01-01T00:00:00.000Z")),
		).toHaveProperty("issues");
	});
...
describe("prop.function()", () => {
	...
	it("should reject non-functions", () => {
		const schema = prop.function();
		expect(schema["~standard"].validate("not a function")).toHaveProperty(
			"issues",
		);
		expect(schema["~standard"].validate({})).toHaveProperty("issues");
	});
...
describe("prop.array()", () => {
	...
	it("should reject non-arrays", () => {
		const schema = prop.array();
		expect(schema["~standard"].validate("not an array")).toHaveProperty(
			"issues",
		);
		expect(schema["~standard"].validate({})).toHaveProperty("issues");
	});
	...
	it("should validate array items with of()", () => {
		...
		expect(schema["~standard"].validate(["a", 1, "c"])).toHaveProperty(
			"issues",
		);
	});
...
describe("prop.tuple()", () => {
	...
	it("should reject tuples with incorrect length", () => {
		...
		expect(schema["~standard"].validate(["x", 1, true])).toHaveProperty(
			"issues",
		);
	});
...
describe("prop.object()", () => {
	...
	it("should reject non-objects", () => {
		const schema = prop.object();
		expect(schema["~standard"].validate("not an object")).toHaveProperty(
			"issues",
		);
...
describe("prop.record()", () => {
	...
	it("should reject non-plain objects", () => {
		...
		expect(schema["~standard"].validate([1, 2])).toHaveProperty("issues");
		expect(
			schema["~standard"].validate(new Date("2026-01-01T00:00:00.000Z")),
		).toHaveProperty("issues");
	});
	...
	it("should preserve hostile keys as safe own properties", () => {
		...
		expect(Object.hasOwn(validated, "prototype")).toBe(true);
		// biome-ignore lint/suspicious/noProto: Exercise an own __proto__ data property to verify prototype-safe handling.
...
describe("prop.union()", () => {
	...
	it("should aggregate issues when all branches fail", () => {
		...
		expect(
			(result as { issues: Array<{ message: string }> }).issues[1].message,
	...
	it("should apply outer union modifiers before branch-level nullish behavior", () => {
		const defaultSchema = prop
			...
			.default("outer");
		expect(defaultSchema["~standard"].validate(undefined)).toEqual({
			value: "outer",
		});
		...
		const optionalSchema = prop
			.union(prop.string(), prop.number().default(7))
			.optional();
		expect(optionalSchema["~standard"].validate(undefined)).toEqual({
			value: undefined,
		});

		const nullableSchema = prop.union(prop.string(), prop.number()).nullable();
...
describe("schema chaining and immutability", () => {
	...
	it("should allow complex chaining", () => {
		...
		expect(schema["~standard"].validate("abcdefghijk")).toHaveProperty(
			"issues",
		);
		expect(schema["~standard"].validate("ABC")).toHaveProperty("issues");
		expect(schema["~standard"].validate("hello")).toEqual({ value: "hello" });
...
describe("integration with validateProps", () => {
	...
	it("should work with prop.enum() in component props", () => {
		...
		expect(() =>
			validateProps(
				{ status: "invalid" } as unknown as { status: "pending" | "active" },
				definitions,
			),
		).toThrow();
	});
	...
	it("should work with prop.union() in component props", () => {
		...
		expect(() =>
			validateProps(
				{ value: true } as unknown as { value: string | number },
				definitions,
			),
		).toThrow();
	});
	...
	it("should work with prop.array().of()", () => {
		...
		expect(() =>
			validateProps({ tags: ["a", "b", "c"] }, definitions),
		).not.toThrow();
		expect(() =>
			validateProps(
				{ tags: ["a", 1, "c"] } as unknown as { tags: string[] },
				definitions,
			),
		).toThrow();
	});
	...
	it("should work with prop.record()", () => {
		...
		expect(() =>
			validateProps({ headers: { authorization: "token" } }, definitions),
		).not.toThrow();
		expect(() =>
			validateProps(
				{ headers: { authorization: 123 } } as unknown as {
	...
	it("should work with prop.date()", () => {
		...
		expect(() =>
			validateProps(
				{ publishedAt: "2026-01-02" } as unknown as { publishedAt: Date },
				definitions,
			),
		).toThrow();
	});
...
describe("exported schema classes", () => {
	...
	it("should export TupleSchema class", () => {
		expect(new TupleSchema([prop.string(), prop.number()])).toBeInstanceOf(
			PropSchema,
		);
	});
	...
	it("should export UnionSchema class", () => {
		expect(new UnionSchema([prop.string(), prop.number()])).toBeInstanceOf(
			PropSchema,
		);
	});
