...
>(): ZoidComponent<P> & {
	...
	const component = vi
		.fn()
		.mockReturnValue(mockInstance) as unknown as ZoidComponent<P> & {
		mockInstance: Partial<ZoidComponentInstance<P>>;
	};
...
describe("createReactComponent", () => {
	...
	it("should create a React component", () => {
		const ReactComponent = createReactComponent(mockComponent, {
			React: mockReact as never,
		});
	...
	it("should mount the ForgeFrame instance into the container and cleanup on unmount", () => {
		...
		expect(mockComponent.mockInstance.render).toHaveBeenCalledWith(
			container,
			"popup",
		);
	...
	it("should run prop-sync effect without dependency array and guard with shallow equality", () => {
		...
		const propSyncDeps = mockReact.useEffect.mock.calls[2]?.[1] as
			| unknown[]
			| undefined;
...
describe("ReactComponentProps", () => {
	...
	it("should accept onRendered callback", () => {
		...
		expect(mockComponent).toHaveBeenCalledWith({});
		expect(mockComponent.mockInstance.event?.once).toHaveBeenCalledWith(
			"rendered",
			expect.any(Function),
		);
	});
	...
	it("should accept onError callback", () => {
		...
		expect(mockComponent.mockInstance.event?.on).toHaveBeenCalledWith(
			"error",
			expect.any(Function),
		);
	});
	...
	it("should pass component-specific props to ForgeFrame component", () => {
		...
		const ReactComponent = createReactComponent(typedMockComponent, {
			React: mockReact as never,
		});
		const container = document.createElement("div");
