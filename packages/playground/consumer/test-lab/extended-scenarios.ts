...
export async function runConfigurationScenario(
	...
	>({
		...
		url: (props) =>
			`${HOST_URL}?scenario=configuration&variant=${encodeURIComponent(props.variant)}`,
		dimensions: (props) => ({
			width: props.variant === "wide" ? 480 : 320,
			height: 320,
		}),
		domain: HOST_ORIGIN,
		props: {
		...
		style: () => ({
			border: "3px solid rgb(233, 69, 96)",
			borderRadius: "12px",
		}),
		...
		results.push(
			assertResult(
				...
				containerTemplateCalls === 1 &&
					prerenderTemplateCalls === 1 &&
					Boolean(
						container.querySelector('[data-test-container="configuration"]'),
					) &&
...
export async function runInstancesScenario(
	sandbox: HTMLElement,
): Promise<TestResult[]> {
	const results: TestResult[] = [];
	...
	try {
		...
		await Promise.all([
			original.render(originalContainer),
			clone.render(cloneContainer),
		]);
		await waitFor(
			() => original.exports?.instanceLabel && clone.exports?.instanceLabel,
		);
		results.push(
			assertResult(
		...
		results.push(
			assertResult(
				...
				original.exports?.peerCount === 1 &&
					clone.exports?.peerCount === 1 &&
		...
		await Promise.all([
			original.close().catch(() => undefined),
			clone.close().catch(() => undefined),
		]);
	}
...
export async function runHostControlsScenario(
	sandbox: HTMLElement,
): Promise<TestResult[]> {
	const results: TestResult[] = [];
	const container = createContainer(sandbox);
	...
	try {
		...
		results.push(
			assertResult(
				...
				controlResult.parentMatched &&
					controlResult.consumerDomain === window.location.origin,
...
export async function runTransportScenario(
	...
	const TrustComponent = ForgeFrame.create<TrustPolicyProps, TransportExports>({
		...
		props: {
			...
			blockedValue: {
				schema: prop.string(),
				trustedDomains: ["https://blocked.example.com"],
			},
	...
	try {
		...
		results.push(
			assertResult(
				...
				!iframe.src.includes("payload=") &&
					!iframe.src.includes("posted-not-queried"),
	...
	try {
		...
		results.push(
			assertResult(
				...
				renderUrlCalls === 1 &&
					new URL(firstFrame.src).searchParams.get("snapshot") === "first",
		...
		results.push(
			assertResult(
				...
				updatedFirstState.label === "first-updated" &&
					updatedFirstState.uid === first.uid,
		...
		results.push(
			assertResult(
				...
				firstContainer.querySelectorAll("iframe").length === 0 &&
					Component.instances.length === 0,
		...
		try {
			await Promise.all([
				alpha.render(alphaContainer),
				beta.render(betaContainer),
			]);
			...
			results.push(
				assertResult(
					...
					alphaInitial.label === "alpha" &&
						betaInitial.label === "beta" &&
						alphaInitial.uid === alpha.uid &&
						betaInitial.uid === beta.uid,
			...
			results.push(
				assertResult(
					...
					alphaUpdated.label === "alpha-updated" &&
						betaUpdated.label === "beta-updated",
	...
	try {
		...
		results.push(
			assertResult(
				...
				total === 12 &&
					notified &&
					callbackMessage === "redirect-callback-complete",
...
export async function runTimeoutRecoveryScenario(
	...
	const TimeoutComponent = ForgeFrame.create<
		ReliabilityProps,
		ReliabilityExports
	>({
	...
	const timedOut = TimeoutComponent({
		scenario: "reliability",
		label: "too-slow",
	});
	let timeoutMessage = "";
	...
	const RecoveryComponent = ForgeFrame.create<
		ReliabilityProps,
		ReliabilityExports
	>({
	...
	const recovered = RecoveryComponent({
		scenario: "reliability",
		label: "recovered",
	});
...
export async function runStressScenario(
	...
	try {
		for (let wave = 0; wave < 5; wave += 1) {
			...
			await Promise.all(
				entries.map(({ container, instance }) => instance.render(container)),
			);
			await Promise.all(
				entries.map(({ instance }) => waitFor(() => instance.exports?.ready)),
			);
			renderedCount += entries.length;

			await Promise.all(
				entries.map(({ instance, label }) =>
			...
			await Promise.all(
				entries.map(async ({ container, instance }) => {
					await instance.close();
		...
		await Promise.all(
			[...active].map((instance) => instance.close().catch(() => undefined)),
		);
	}
...
interface CheckoutExports {
	...
	submitPayment: (token: string) => Promise<
		CheckoutReceipt & {
			status: string;
...
export async function runCheckoutE2EScenario(
	sandbox: HTMLElement,
): Promise<TestResult[]> {
	const results: TestResult[] = [];
	const container = createContainer(sandbox);
	...
	for (const event of ["prerender", "render", "rendered", "display"]) {
		instance.event.on(event as "prerender", () => {
	...
	try {
		...
		results.push(
			assertResult(
				...
				container.querySelectorAll("iframe").length === 1 &&
					instance.exports?.checkoutReady === true,
		...
		results.push(
			assertResult(
				...
				approvedReceipt?.transactionId === "txn-browser-e2e" &&
					approvedReceipt.amount === 49.95,
		...
		results.push(
			assertResult(
				...
				receipt.transactionId === "txn-browser-e2e" &&
					receipt.amount === 49.95 &&
					receipt.tokenAccepted &&
					receipt.sessionId === "session-browser-e2e",
		...
		results.push(
			failure("Customer checkout end-to-end scenario completed", error),
		);
		await instance.close().catch(() => undefined);
	}
