...
async function runLifecycleScenario(
	...
	instance.event.once("prerender", () => {
		...
		void instance
			.updateProps({ value: "during-render" })
			.catch((error: Error) => {
	...
	try {
		...
		if (!reentrantRender)
			throw new Error("Lifecycle callback did not re-enter render");
	...
	try {
		...
		results.push(
			assertResult(
				...
				callbackArgs?.[0] === 19 &&
					callbackArgs[1] === 23 &&
					instance.exports?.callbackResult === 42,
...
async function runPropsScenario(sandbox: HTMLElement): Promise<TestResult[]> {
	...
	const Component = ForgeFrame.create<PropsScenario>({
		...
		props: {
			...
			value: prop.string(),
			onObservedProps:
				prop.function<(props: Record<string, unknown>) => void>(),
		},
	});
	const instance = Component({
		scenario: "props",
...
async function runControlsScenario(
	sandbox: HTMLElement,
): Promise<TestResult[]> {
	const results: TestResult[] = [];
	const container = createContainer(sandbox);
	...
	try {
		...
		results.push(
			failure("Consumer controls browser scenario completed", error),
		);
		await instance.close().catch(() => undefined);
	}
	...
	try {
		...
		results.push(
			assertResult(
				...
				instance.exports?.nestedReady === true &&
					instance.exports.childExportReady === true,
				`nested ready: ${String(instance.exports?.nestedReady)}; child export: ${String(instance.exports?.childExportReady)}`,
			),
		);
	} catch (error) {
		results.push(
			failure("Nested components browser scenario completed", error),
		);
	} finally {
		await instance.close().catch(() => undefined);
	}
